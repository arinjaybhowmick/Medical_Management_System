...
public class JwtGeneratorValidator {
    ...
    // private String createToken(Map<String, Object> claims, Authentication
    // authentication) {
    // String role =authentication.getAuthorities().stream()
    // .map(r -> r.getAuthority()).collect(Collectors.toSet()).iterator().next();
    // return
    // Jwts.builder().claim("role",role).setSubject(authentication.getName()).setIssuedAt(new
    ...
    private String createRefreshToken(Map<String, Object> extraClaims, Authentication authentication) {
        String role = authentication.getAuthorities().stream()
                .map(r -> r.getAuthority()).collect(Collectors.toSet()).iterator().next();
        return Jwts.builder().setClaims(extraClaims).claim("role", role).setSubject(authentication.getName())
                .setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(60 * 24 * 2)))
